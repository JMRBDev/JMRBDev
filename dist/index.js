...
var rss_parser = __nccwpck_require__(6946);
...
const replaceMd = (filepath, newContent) => {
  const data = external_fs_.readFileSync(filepath, { encoding: "utf8", flag: "r" });
  const fileContents = data.toString();

  const newFileContent = spliceMd(fileContents, newContent);

  external_fs_.writeFileSync(filepath, newFileContent);
};
